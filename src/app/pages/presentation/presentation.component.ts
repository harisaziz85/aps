...
export class PresentationComponent implements OnInit {
 ...
 async generateReport(): Promise<void> {
  ...
  // Use a CORS proxy (temporary for testing; consider a custom proxy for production)
  ...
  const normalizeUrl = (url: string): string => {
    ...
    if (url.startsWith('https://aps-app-frontend.vercel.app/') || url.startsWith('https://vps.allpassiveservices.com.au/')) {
      return url;
    }
    // Handle relative paths
    const cleanPath = url.replace(/^\/*uploads\/*/i, '').replace(/^\/+/, '');
    return `${corsProxy}https://vps.allpassiveservices.com.au/uploads/${cleanPath}`;
  };
  ...
  const loadImage = async (url: string): Promise<string> => {
    ...
    if (normalizedUrl.includes('via.placeholder.com')) {
      return normalizedUrl; // Return placeholder directly
    }
    try {
  ...
  for (const row of tableData) {
    ...
    for (const header of headers) {
      ...
      if (header === 'Plan') {
        ...
        try {
          const photoPromises = row['Photos'].slice(0, 1).map(url => loadImage(normalizeUrl(url)));
