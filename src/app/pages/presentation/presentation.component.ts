...
export class PresentationComponent implements OnInit {
 ...
 async generateReport(): Promise<void> {
  ...
  const normalizeUrl = (url: string): string => {
    ...
    // Remove any existing base URL or incorrect prefixes
    const cleanPath = url
      .replace(/^https?:\/\/vps\.allpassiveservices\.com\.au\/?/, '')
      .replace(/^\/*uploads\/*/i, '')
      .replace(/^\/+/, '');
    // Always prepend the correct backend base URL
  ...
  const loadImage = async (url: string): Promise<string> => {
    ...
    try {
      const response = await fetch(normalizedUrl, {
        ...
        credentials: 'same-origin', // Adjust based on your auth requirements
      });
      if (!response.ok) {
  ...
  // Add logo
  try {
    const logoUrl = 'images/logo.png'; // Ensure this path is correct for your live environment
