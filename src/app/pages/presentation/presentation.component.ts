...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule, SvgIconsComponent, FootComponent, ImageModalComponent],
...
export class PresentationComponent implements OnInit {
  ...
  getAttributeDisplayValue(fieldKey: string): string {
    if (this.selectedInstance?.attributes) {
      const attr = this.selectedInstance.attributes.find(a => a.name.toLowerCase() === fieldKey.toLowerCase());
      return attr ? attr.selectedValue || 'N/A' : 'N/A';
    }
    return 'N/A';
  }

  getAttributeDisplayValues(fieldKey: string): string[] {
    return this.attributeValues[fieldKey.toLowerCase()] || ['N/A'];
  }
  ...
  fetchReports(): void {
    ...
    this.http.get<any[]>(`https://vps.allpassiveservices.com.au/api/project/reports/${this.projectId}`).subscribe({
      next: (response) => {
  ...
  getReportCount(projectId: string): number {
    return this.reports.filter(report => report.projectId === projectId).length;
  }
  ...
  async generateReport(): Promise<void> {
    ...
    if (this.selectApproval && this.selectedApprovalDocument) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(10);
      const docName = this.approvalDocuments.find(doc => doc.fileUrl === this.selectedApprovalDocument)?.name || 'Document';
      doc.text(docName, margin, yOffset);
      yOffset += lineHeight;

      const approvalUrl = normalizeUrl(this.selectedApprovalDocument);
      try {
        const imgData = await loadImage(approvalUrl);
        checkPageBreak(imageHeight + lineHeight);
        doc.addImage(imgData, 'PNG', margin, yOffset, imageWidth, imageHeight);
        ...
        checkPageBreak(lineHeight);
        doc.setTextColor(255, 0, 0);
        doc.text(`Failed to load approval document image: ${approvalUrl}`, margin, yOffset);
        doc.setTextColor(0, 0, 0);
        yOffset += lineHeight;
      }
    }

    doc.save(`${projectDetails.projectName || 'report'}.pdf`);
