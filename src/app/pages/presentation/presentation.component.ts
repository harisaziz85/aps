...
export class PresentationComponent implements OnInit {
  ...
  private resizeCanvas() {
    ...
    if (this.image.width && this.image.height) {
      canvas.width = Math.min(this.image.width, window.innerWidth * 0.8);
      canvas.height = (canvas.width / this.image.width) * this.image.height;
      console.log('Canvas resized:', canvas.width, canvas.height);
    } else {
      console.warn('Image dimensions not available, setting default size');
      canvas.width = 600;
      canvas.height = 400;
    }
  }
  ...
  drawImage() {
    ...
    if (this.image.complete && this.image.src && !this.image.src.includes('via.placeholder.com')) {
      this.ctx.drawImage(this.image, 0, 0, canvas.width, canvas.height);
      console.log('Image drawn on canvas:', this.image.src);
    } else {
      console.warn('Image not ready or invalid, drawing placeholder');
      this.ctx.fillStyle = '#f0f0f0';
      this.ctx.fillRect(0, 0, canvas.width, canvas.height);
      this.ctx.fillStyle = '#000';
      this.ctx.font = '20px Arial';
      this.ctx.textAlign = 'center';
      this.ctx.fillText('Image not available', canvas.width / 2, canvas.height / 2);
    }
  ...
  handleEditOption(event: Event) {
    ...
    } else if (value === 'circle' || value === 'line' || value === 'text' || value === 'arrow' || value === 'none') {
      this.setDrawingMode(value as 'none' | 'circle' | 'line' | 'text' | 'arrow');
    }
    this.cdr.detectChanges();
  }
  ...
  openImageModal(imageUrl: string): void {
    this.selectedImage = this.normalizeUrl(imageUrl);
    ...
    this.image.onerror = () => {
      ...
      this.image.src = 'https://via.placeholder.com/600x400?text=Image+Error';
      this.resizeCanvas();
      this.drawImage();
      this.cdr.detectChanges();
    };
  ...
  saveChanges() {
    ...
    // Optionally, you can send the dataUrl to a server or store it
  ...
  async generateReport(): Promise<void> {
    ...
    for (const row of tableData) {
      ...
      const photosHeight = row['Photos'].length > 0 ? photoImageHeight * Math.min(row['Photos'].length, 3) : 6;
      ...
      for (const header of headers) {
        ...
        if (header === 'Plan') {
          ...
          try {
            const photoPromises = row['Photos'].slice(0, 3).map(url => loadImage(url));
